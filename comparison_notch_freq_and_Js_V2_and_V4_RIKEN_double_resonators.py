...
measured_notches = np.array([8., 9.3, 8.8, 8.0]) * 2*np.pi * 1e9 #- 0.1* 2*np.pi * 1e9 ##8.27
measured_omega_rs = np.array([10385,10666,10541,10247]) * 2*np.pi * 1e6
measured_omega_ps = np.array([10407,10712,10567,10231]) * 2*np.pi * 1e6
...
print('notch agreement V4:', (predicted_notches - measured_notches)/measured_notches * 100)
...
J_measured = np.array([39, 27, 31, 36])

#np.array([2, 2, 3, 1.1])

plt.plot(J_preds/(2*np.pi*1e6), J_measured, linestyle = 'none', marker = 'o', color = color0, markersize = 8)
print('J agreement V4:', (J_preds/(2*np.pi*1e6) - J_measured)/J_measured * 100)
