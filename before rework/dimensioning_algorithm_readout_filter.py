...
from coupled_transmission_line_util import lambda_by_4_f, find_notch_filter_frequency, lumped_elements_get_j, get_lumped_elements,  lumped_element_Z21
...
def function(inputs, target):
    ...
    f1 = lambda_by_4_f(l_Gn+l_c+l_Gf)
    f2 = lambda_by_4_f(l_Rn+l_c+l_Rf)
    ...
    j_coupling = lumped_elements_get_j(f1*2*np.pi, f2*2*np.pi, l_c, l_Gf, l_Gn, l_Rf, l_Rn, Lm_per_len, Cm_per_len)
...
def print_message(sol, target, plot=0):
    ...
    if plot==1:
        ...
        C1, L1, C2, L2, Cg, Lg = get_lumped_elements(solution[2], solution[1], solution[0], solution[4], solution[3], get_Lm(solution[5]), get_Cm(solution[5]),  phase_vel=119919602, Z0=64.6)
