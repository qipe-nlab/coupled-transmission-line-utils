...
test_Z_equiv_LE_circuit = Z_transfer_equivalent_LE_circuit(l_c, l_Gf, l_Gn, l_Rf, l_Rn, Lm, Cm, omegas, phase_vel=3*10**8/2.5, Z0=65, receiver_type='lambda/2')
...
plt.vlines(test_notch_freq_analytic*2/(2*np.pi*1e9), 0.01, 10, color = 'b', linestyle = '--')
plt.yscale('log')
...
# J_val_test = J_coupling(l_c, l_Gf, l_Gn, l_Rf, l_Rn, Lm, Cm, phase_vel=3*10**8/2.5, Z0=65, receiver_type='lambda/2')
# J_val_test_symbolic = J_coupling_analytic(l_c, l_Gf, l_Gn, l_Rf, l_Rn, Cm, phase_vel=3*10**8/2.5, Z0=65, receiver_type='lambda/2')

# print('J_val_test:', J_val_test / (2*np.pi*1e6))
# print('J_val_test_symbolic:', J_val_test_symbolic / (2*np.pi*1e6))

Z0 = 65
phase_vel=3*10**8/2.5

d_vals = np.linspace(1, 8, 5) * 1e-6

J_vals_solve_eq_c = np.array([J_coupling(l_c, l_Gf, l_Gn, l_Rf, l_Rn, cap.get_Lm(d), cap.get_Cm(d), phase_vel=phase_vel, Z0=Z0, receiver_type='lambda/2') for d in d_vals])
print('J_vals_solve_eq_c:', J_vals_solve_eq_c/(2*np.pi*1e9))

J_vals_analytic = np.array([J_coupling_analytic(l_c, l_Gf, l_Gn, l_Rf, l_Rn, cap.get_Cm(d_val), phase_vel=phase_vel, Z0=Z0, receiver_type='lambda/2') for d_val in d_vals])
print('J_vals_analytic:', J_vals_analytic/(2*np.pi*1e9))

plt.plot(d_vals * 1e6, J_vals_solve_eq_c/(2*np.pi*1e6), color = 'r')
