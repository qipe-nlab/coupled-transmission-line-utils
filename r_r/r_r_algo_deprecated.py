...
def cost_fl_vector(inputs, target, Cs, calibration_len, weighted_error=0):
    #print(inputs)
...
def cost_fl_scalar(inputs, target, Cs, calibration_len, weighted_error=1):
    l_Gn = inputs[0] +calibration_len/2
    l_Gf = inputs[1] +calibration_len/2
    l_c = inputs[2]
    l_Rn = inputs[3] +calibration_len/2
...
def cost_ll_scalar(inputs, target, Cs, calibration_len, weighted_error=1):
    l_Gn = inputs[0] +calibration_len/2
    l_Gf = inputs[1] +calibration_len/2
    l_c = inputs[2]
    l_Rn = inputs[3] +calibration_len/2
    ...
    # Resonator lengths
    #
    len_r2 = l_Rn+l_c+l_Rf-calibration_len
    len_r1 = l_Gn+l_c+l_Gf-calibration_len
    ...
    total_error = ((len_r1-target[0]))**2 + ((len_r2-target[1]))**2 + (1e-12*(notch_f-target[2]))**2 + (1e-9*(j_coupling-target[3]))**2
    #print("f1 = ", f1/1e9)
    #print("len_r2 = ", len_r2*1e6)
    #print("target[2] = ", target[1]*1e6 )
    return total_error
...
def cost_ll_vector(inputs, target, Cs, calibration_len, weighted_error=0):
    #print(inputs)
    l_Gn = inputs[0]+calibration_len/2
    l_Gf = inputs[1]+calibration_len/2
    l_c = inputs[2]
    ...
    #print([(f1-target[0])/1e6, (len_r2-target[1])*1e6, (notch_f-target[2])/1e6, (j_coupling-target[3])/1e6, constraint, 0])
    weights=[1,1,1,1,1]
    if weighted_error: return [(len_r2-target[0])*weights[0], (len_r2-target[1])*weights[1], (notch_f-target[2])*weights[2], (j_coupling-target[3])*weights[3], (constraint)*weights[4], 0]
    
    return [(len_r1-target[0]), len_r2-target[1], notch_f-target[2], j_coupling-target[3], constraint, 0]
...
def print_message(sol, target, Cs, calibration_len, plot=0):
    solution = sol.x
    result = cost_rl_vector(solution, [0,0,0,0], Cs, calibration_len, weighted_error=0)
...
def print_message_alternative(sol, target, Cs, calibration_len, plot=0):
    solution = sol.x
    result = cost_ll_vector(solution, [0,0,0,0], Cs, calibration_len, weighted_error=0)
    ...
    print("-------------------------------")
    print("         Results in")
    print(" len_r1 = ", int(result[0]*1e7)/10, "[um]")
    print(" len_r2 = ", int(result[1]*1e7)/10, "[um]")
    print(" fn = ", int(result[2]*1e-6)/1000, "[GHz]")
    print(" J = ", int(result[3]*1e-3/2/np.pi)/1000, "[MHz]")
    print("################################")
    ...
    if plot==1:
        ...
        plt.plot(omegas/(2*np.pi*1e9), Zs, label="lumped element model", color="blue")
        plt.vlines([cf.lambda_by_4_f(target[0]+calibration_len)*1e-9,cf.lambda_by_4_f(target[1]+calibration_len)*1e-9], 1e-3, 1e5, color="orange", label="resonators")
        plt.vlines([target[2]*1e-9], 1e-3, 1e5, color="red", label="notch")
        plt.yscale("log")
        plt.show()
    return 0
...
def solve_for_r_r(target, x0, Cs, calibration_len, show=0, plot=0):
    ...
    #
    sol = opt.differential_evolution(cost_fr_scalar, args=(target, Cs, calibration_len), bounds=bounds, callback=print_progress1)
...
# Use two lengths as target
def solve_for_r_r_alternative(target, x0, Cs, calibration_len, bounds=[(750e-6,1500e-6),(10e-6,2500e-6),(200e-6,400e-6),(750e-6,1500e-6),(10e-6,2500e-6),(3e-6,10e-6)], show=0, plot=0):
    print("\n")
    print("Started solving for r_r...")
    #sol = opt.root(function, x0, args=(target, Cs))
    #sol = opt.differential_evolution(cost_ll_scalar, args=(target, Cs, calibration_len), bounds=bounds, callback=print_progress1)
    #sol = opt.minimize(cost_ll_scalar, args=(target, Cs, calibration_len), x0=x0, bounds=bounds, callback=print_progress2)
    sol = opt.brute(cost_ll_scalar, ranges=bounds, args=(target, Cs, calibration_len), Ns=10)
    if show!=0:
        print_message_alternative(sol, target, Cs, calibration_len, plot=plot)
    return sol
